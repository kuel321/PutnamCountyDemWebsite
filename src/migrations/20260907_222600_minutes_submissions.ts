...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`minutes_submissions\` (
  	...
  	\`review_notes\` text,
  	\`published_minutes_id\` integer,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	FOREIGN KEY (\`meeting_id\`) REFERENCES \`meetings\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`submitted_by_id\`) REFERENCES \`club_members\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`published_minutes_id\`) REFERENCES \`meeting_minutes\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`minutes_submissions_meeting_idx\` ON \`minutes_submissions\` (\`meeting_id\`);`)
  await db.run(sql`CREATE INDEX \`minutes_submissions_submitted_by_idx\` ON \`minutes_submissions\` (\`submitted_by_id\`);`)
  await db.run(sql`CREATE INDEX \`minutes_submissions_published_minutes_idx\` ON \`minutes_submissions\` (\`published_minutes_id\`);`)
  await db.run(sql`CREATE INDEX \`minutes_submissions_updated_at_idx\` ON \`minutes_submissions\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`minutes_submissions_created_at_idx\` ON \`minutes_submissions\` (\`created_at\`);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`minutes_submissions_id\` integer REFERENCES minutes_submissions(id);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_minutes_submissions_id_idx\` ON \`payload_locked_documents_rels\` (\`minutes_submissions_id\`);`)
}

export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`minutes_submissions\`;`)
